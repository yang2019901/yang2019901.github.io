...
这是我于2022年7月开始，历时一年独立开发完成的Unity仿真器，旨在模拟RoboMaster2022超级对抗赛的完整比赛机制。具体来说，该模拟器模拟了两组多兵种机器人队伍在复杂地形上完成射击对抗的类MOBA比赛，是一个大型的系统工程。针对上百页规则手册和复杂的场地机制（车-车交互，车-场地交互），我运用解耦和模块化的开发策略，通过软件工程的中介者模式减少不同模块的依赖，并利用继承与虚函数等技术构建了整个工程的代码框架。除此之外，我运用轮式碰撞体实现带弹簧减震的车辆运动模拟，解决传统角色控制器方案不真实的缺陷；通过虚拟云台方案模拟了实际的电气滑环结构，既模拟了云台与底盘的固连，又避免了传统方案的随动问题，进而完美模拟了小陀螺机制和自动瞄准系统；基于Mirror插件编写多主机的机器人与场景状态同步代码，通过抽取场景状态的最小完备集合，减少带宽需求约90%，实现基于ipv6的高性能多人联机方案。

完整工程的源代码托管于Github，预构建的可执行程序可通过Release页面下载。
